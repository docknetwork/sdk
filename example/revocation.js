...
const controllerDID = DockDid.random();
...
async function createRegistry() {
  ...
  console.log(
    `Creating a registry with owner DID (${controllerDID}) with policy type:`,
...
async function unrevoke(pair) {
  ...
  const extrinsic = await dock.revocation.unrevokeCredentialWithOneOfPolicy(
    registryId,
    revokeId,
    controllerDID,
    pair,
...
async function revoke(pair) {
  ...
  const extrinsic = await dock.revocation.revokeCredentialWithOneOfPolicy(
    registryId,
    revokeId,
    controllerDID,
    pair,
...
async function main() {
  ...
  const pair = new DidKeypair(
    dock.keyring.addFromUri(controllerSeed, null, "sr25519"),
    1,
  );

  // Set our controller DID and associated keypair to be used for generating proof
  console.log(
    `Creating controller DID (${controllerDID}) using sr25519 pair from seed (${controllerSeed})...`,
  );
  ...
  if (isRevoked) {
    ...
    const isUnrevoked = !(await dock.revocation.getIsRevoked(
      registryId,
      revokeId,
    ));
    if (isUnrevoked) {
