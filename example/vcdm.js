...
async function setup() {
  ...
  const pair1 = new DidKeypair(
    dock.keyring.addFromUri(holderSeed, null, "ed25519"),
    1,
  );
  ...
  policy.addOwner(DockDid.from(issuerDID));
  ...
  try {
    ...
    if (verifyResult.verified) {
      ...
      if (ver.verified) {
        ...
        console.error(
          "Presentation could not be verified!. Got error",
          ver.error,
        );
        process.exit(1);
      }
      ...
      console.error(
        "Credential could not be verified!. Got error",
        verifyResult.error,
      );
      process.exit(1);
    }
