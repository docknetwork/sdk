...
export default function generateOffchainSignatureModuleTests(
  ...
  describe(`Checks ${didModule.constructor.name} and ${offchainSignatures.constructor.name} with all public keys and params`, () => {
    it.only("Generates a `DIDDocument` with `OffchainPublicKey` and creates a `OffchainParameters` owned by this DID", async () => {
  ...
  describe.each(cases)(
    ...
    ({ module, PublicKey, Params }) => {
      it.skip(`Generates a \`DIDDocument\` with \`OffchainPublicKey\` and creates a \`OffchainParameters\` owned by this DID using ${module.constructor.name} with ${PublicKey.name} and ${Params.name}`, async () => {
        const did = Did.random();
