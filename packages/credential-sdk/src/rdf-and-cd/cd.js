...
async function credToEECG(expandedCredential) {
  ...
  assert(
    cred[expandedIssuerProperty] !== undefined,
    'encountered credential without an issuer',
  );
  ...
  for (const claim of cg) {
    ...
    assert(
      deepEqual(claim[3], { DefaultGraph: true }),
      'subgraph found in credential',
    );
    claim[3] = { Iri: issuer };
  }
...
export function extractProof(expandedPresentation) {
  return jsonld
    .getValues(unwrapE(expandedPresentation), expandedLogicProperty)
    .map(fromJSONLiteral)
    .flat(1);
}
 ...
 */
export async function proveCompositeClaims(
...
export function proveh(premises, toProve, rules) {
...
export function validateh(rules, proof) {
  foreachQuadInRules(rules, assertQuad);
  const { assumed, implied } = validate(canonRules(rules), canonProof(proof));
  return {
...
function assertQuad(quad) {
  assert(
    quad.length === 4,
    () => `Quads must have length 4, got: ${JSON.stringify(quad)}`,
  );
}
