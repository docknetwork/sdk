...
export function getJsonSchemaFromCredential(credential, full = false) {
  ...
  if (typeof credential.credentialSchema.id !== 'string') {
    throw new Error(
      `credentialSchema was expected to be string but was ${typeof credential.credentialSchema}`,
    );
  }
...
export function getPrivateStatus(credential) {
  return credential.credentialStatus
    && credential.credentialStatus.type === PrivateStatusList2021EntryType
 ...
 */
export async function verifyPrivateStatus(
  credentialStatus,
  ...
  if (typeof expectedIssuer === 'string') {
    const issuer = typeof privateStatusListCredential.issuer === 'object'
      ? privateStatusListCredential.issuer.id
      : privateStatusListCredential.issuer;
    if (issuer !== expectedIssuer) {
      throw new Error(
        `Expected issuer to be ${expectedIssuer} but found ${issuer}`,
      );
    }
  }
  ...
  if (verifyStatusListCredential) {
    const verifyResult = await verifyCredential(
      privateStatusListCredential.toJSON(),
      {
        suite: fullSuite,
        documentLoader,
      },
    ...
    if (!verifyResult.verified) {
      throw new Error(
        `Status list credential failed to verify with error: ${verifyResult.error}`,
      );
    }
  }
...
function validateStatusPurpose(purpose) {
  if (purpose !== 'suspension' && purpose !== 'revocation') {
    throw new Error(
      `statusPurpose must 'suspension' or 'revocation' but was '${purpose}'`,
    );
  }
}
