...
export default class EcdsaSecp256r1VerificationKey2019 {
  ...
  static from(verificationMethod) {
    if (
      ...
      || verificationMethod.type.indexOf(EcdsaSecp256r1VerKeyName) === -1
    ) {
      throw new Error(
        `verification method should have type ${EcdsaSecp256r1VerKeyName} - got: ${verificationMethod.type}`,
      );
    }
  ...
  verifier() {
    return EcdsaSecp256r1VerificationKey2019.verifierFactory(this.publicKey);
  }

  static verifierFactory(publicKey) {
    return {
      async verify({ data, signature }) {
        return Secp256r1Keypair.verify(data, signature, publicKey);
      },
    };
  }
}
