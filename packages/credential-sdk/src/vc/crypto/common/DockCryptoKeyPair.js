  ...
  class DockCryptoKeyPair {
    constructor(options, type) {
      ...
      if (keypair) {
        ...
        this.privateKeyBuffer = options.privateKeyBase58
          ...
          : undefined;
        this.publicKeyBuffer = b58.decode(String(options.publicKeyBase58));
      }
    }
    ...
    static generate({
      ...
      const keypair = this.KeyPair.generate(
        params || this.paramGenerator(msgCount, this.defaultLabelBytes),
        seed,
      );
    ...
    static signerFactory(key) {
      ...
      const { SecretKey, Signature, defaultLabelBytes } = this;
    ...
    static verifierFactory(keypair) {
      ...
      const { PublicKey, Signature, defaultLabelBytes } = this;
      const keypairClass = this;
