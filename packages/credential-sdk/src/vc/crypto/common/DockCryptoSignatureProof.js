  ...
  class DockCryptoSignatureProof extends CustomLinkedDataSignature {
    ...
    async verifyProof({
      ...
      try {
        ...
        if (!res.verified) {
          throw new Error(
            `Invalid anoncreds presentation due to error: ${res.error}`,
          );
        }
    ...
    static derivedToAnoncredsPresentation(document) {
      ...
      return {
        ...
        version:
          proof.presVersion !== undefined ? proof.presVersion : proof.version,
        nonce: proof.nonce,
