...
describe('Composite claim soundness checker', () => {
  test('control: issue and verify', async () => {
    ...
    console.log(vresult);
    expect(vresult).toHaveProperty('verified', true);
    credential.issuanceDate = '9010-01-01T19:23:24Z';
    expect(await verifyC(credential)).toHaveProperty('verified', false);
  });
  ...
  test('assumption: credential with false issuer will fail', async () => {
    ...
    await modifyDocument(`${issuerb}#keys-1`, (original) => ({
      ...original,
      controller: issuera,
    }));
    expect(
  ...
  test('Proof including unstated claims should fail.', async () => {
    ...
    await expect(checkSoundness(presentation, rules)).rejects.toHaveProperty(
  ...
  test('Proof including inapplicable rule should fail.', async () => {
    ...
    await expect(checkSoundness(presentation, rules)).rejects.toEqual({
  ...
  test('Unverifiable credential should fail.', async () => {
    ...
    presentation[expandedLogicProperty] = jsonLiteral([
      { rule_index: 0, instantiations: [] },
    ]);
    await expect(
      checkSoundness(presentation, rules).catch((err) => Promise.reject(JSON.stringify(err))),
  ...
  test('bddap is named Gorgadon because joe is a pig that can fly', async () => {
    ...
    const joe_is_a_pig = await issueCredential(
      pigchecker_kp,
      {
    ...
    const joe_can_fly = await issueCredential(
      faa_kp,
      {
        ...
        credentialSubject: {
          '@id': 'did:example:joe',
          'https://example.com/Ability': {
            '@id': 'https://example.com/Flight',
          },
        },
      },
      true,
    ...
    const proof = proveh(presentation_claimgraph, [claim_to_prove], rules);
...
async function validCredential() {
  ...
  const credential = await issueCredential(kp, cred, true, documentLoader);
  expect(await verifyC(credential)).toHaveProperty('verified', true);
  return credential;
}
