...
const dbpediaSource = [
  [{ Iri: "a" }, { Iri: "b" }, { Iri: "http://dbpedia.org/resource/Belgium" }],
  [{ Iri: "c" }, { Iri: "d" }, { Iri: "http://dbpedia.org/resource/Ghent" }],
  [{ Iri: "c" }, { Iri: "d" }, { Iri: "http://dbpedia.org/resource/Austria" }],
];
...
describe("RDF SPARQL", () => {
  ...
  test("Can query a claimgraph", async () => {
    ...
    const queryResult = await queryNextLookup(claimgraph, query, engine);
    expect(queryResult).toEqual([{ Iri: "did:b" }]);
  });
});
