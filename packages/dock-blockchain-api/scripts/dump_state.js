...
import { asDockAddress } from "@docknetwork/credential-sdk/utils";
...
async function downloadBbsPlus() {
  ...
  return {
    bbsPlusParamsCounter: storageMapEntriesToObject(paramsCounter),
    bbsPlusParams: storageDoubleMapEntriesToObject(params),
    bbsPlusKeys: storageDoubleMapEntriesToObject(keys),
  };
}

async function downloadAccum() {
  const counter =
    await dock.api.query.accumulator.accumulatorOwnerCounters.entries();
  ...
  return {
    accumulatorCounter: storageMapEntriesToObject(counter),
    accumulatorParams: storageDoubleMapEntriesToObject(params),
    accumulatorKeys: storageDoubleMapEntriesToObject(keys),
    accumulators: storageMapEntriesToObject(accum),
  };
}
...
async function downloadState() {
  ...
  const { dids, controllers, keys, serviceEndpoints } = await downloadDids();
