...
export async function addOracles(
  web3,
  signer,
  contractAddr,
  contract,
  ...
  const addOracle1Code = contract.methods
    .changeOracles([], [oracle1.address], [oracle1.address], 1, 1, 0)
    .encodeABI();
  await sendEVMTxn(web3, signer, contractAddr, addOracle1Code);
  console.log(`Oracles: ${await contract.methods.getOracles().call()}`);

  const addOracle2Code = contract.methods
    .changeOracles([], [oracle2.address], [oracle2.address], 1, 2, 0)
    .encodeABI();
