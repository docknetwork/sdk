...
async function setupVotingExecutor(web3, signer) {
  ...
  const incrementCall = counterContract.methods
    .increment()
    .encodeABI()
    .slice(2);
  const decrementCall = counterContract.methods
    .decrement()
    .encodeABI()
    .slice(2);
...
async function main() {
  ...
  const daoAddr = await createNewDao(
    web3,
    alice,
    alice.address,
    daoFactContractAddr,
  ...
  const votingAppAddress = await setupVotingApp(
    web3,
    alice,
    alice.address,
    appId,
  ...
  const tokenContractAddr = await deployToken(web3, alice, [
    [bob.address, 51],
    [carol.address, 29],
    [dave.address, 20],
  ]);
  ...
  const voteId = await createNewVote(
    web3,
    carol,
    votingAppAddress,
    incrementScript,
