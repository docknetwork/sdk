...
async function sendOnChainDIDTxns(count, waitForFinalization = true) {
  ...
  while (txs.length < count) {
    ...
    const tx = dock.modules.did.createNewOnchainTx(did, [didKey], []);
    txs.push(tx);
    didPairs.push(new DidKeypair([did, 1], pair));
  }
...
async function sendAddKeyTxns(count, didPairs) {
  ...
  let j = 0;
  while (txs.length < count) {
    const did = didPairs[j].did;
    const currentPair = didPairs[j];
    const seed = randomAsHex(32);
...
async function sendRemoveTxns(count, didPairs, waitForFinalization = true) {
  ...
  while (txs.length < count) {
    ...
    const tx = await dock.modules.did.createRemoveTx(did, did, currentPair);
    txs.push(tx);
    j++;
  }
