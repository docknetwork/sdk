...
import { getStateChange } from "../../src/utils";
 ...
 * @returns {Promise<()>}
 */
async function submit({ nodeWsUrl, payerKey, proposal, votes }) {
...
function toAuth(votes) {
  return votes.map((did, keyId, signature, nonce) => [
    createDidSig(did, keyId, signature),
    nonce,
  ]);
}
...
async function assertValidAuth(nodeClient, proposal, mpauth) {
  ...
  for (const [didSig, nonce] of mpauth) {
    ...
    const encoded_state_change = asEncodedStateChange(
      nodeClient,
      roundNo,
      proposal,
      nonce
