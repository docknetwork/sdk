...
import { isHexWithGivenByteSize } from '@docknetwork/credential-sdk/utils';
...
export async function blockNumberToHash(api, maybeNumber) {
 ...
 */
export async function getAllExtrinsicsFromBlock(
  api,
  numberOrHash,
  includeFailedExtrinsics = true,
) {
  const block = await getBlock(api, numberOrHash);
  let { extrinsics } = block;
  if (!includeFailedExtrinsics) {
    ...
    const events = await getAllEventsFromBlock(
      api,
 ...
 * @param {*} formatted
 */
export async function getAllEventsFromBlock(
  api,
  numberOrHash,
  formatted = true,
) {
...
export async function getTransferEventsFromBlock(
  api,
  numberOrHash,
  formatted = true,
) {
  ...
  return events.filter(
    (event) => event.event && event.event.index === '0x0302',
  );
}
...
function formatBalIfNeeded(bal, format = true) {
  return format
    ? formatBalance(bal, { decimals: DECIMAL, withSi: true, withUnit: SYMBOL })
    : bal.toNumber();
}
 ...
 */
export async function getTransferExtrinsicsFromBlock(
  api,
  numberOrHash,
...
function checkValidMicroAmount(micros) {
  ...
  if (!Number.isSafeInteger(micros) || micros < 0) {
    throw new Error(
      `Amount must be a positive integer and must fit in 53 bits but was ${micros}`,
    );
  }
}
 ...
 * @returns {Promise<*>}
 */
export async function transferDock(
  api,
  senderKeypair,
  recipAddr,
  amount,
  ...
  return transferMicroDock(
    api,
    senderKeypair,
    recipAddr,
    docksToMicroDocks(amount),
    sendTxn,
  );
}
 ...
 * @returns {Promise<*>}
 */
export async function transferMicroDock(
  api,
  senderKeypair,
  recipAddr,
  amount,
  ...
  return sendTxn
    ? txn.signAndSend(senderKeypair)
    : txn.signAsync(senderKeypair);
}
