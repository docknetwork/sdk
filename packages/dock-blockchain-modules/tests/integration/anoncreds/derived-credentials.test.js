...
describe.each(Schemes)(
  ...
  ({
    ...
    const posAccumulatorId = DockAccumulatorId.random(did1);
    const posAccumState = new InMemoryState();
    let posAccumKeypair;
    let posAccumWitness;

    const posKvAccumulatorId = DockAccumulatorId.random(did1);
    const posKvAccumState = new InMemoryState();
    let posKvAccumSecretKey;
    let posKvAccumWitness;

    const uniAccumulatorId = DockAccumulatorId.random(did1);
    const uniAccumState = new InMemoryKBUniversalState();
    let uniAccumKeypair;
    let uniAccumWitness;

    const uniKvAccumulatorId = DockAccumulatorId.random(did1);
    ...
    beforeAll(async () => {
      ...
      pair1 = new DidKeypair([did1, 1], Ed25519Keypair.random());
      ...
      const bytes1 = posAccumKeypair.publicKey.value;
      const posAccumPk = AccumulatorModule.prepareAddPublicKey(bytes1);
      await modules.accumulator.addPublicKey(null, posAccumPk, did1, pair1);
      ...
      await modules.accumulator.addPublicKey(null, uniAccumPk, did1, pair1);
      ...
      const accumulator2 = await KBUniversalAccumulator.initialize(
        members,
        params,
    ...
    test(`For ${Name}, persist credential status using VB positive accumulator with keyed-verification when deriving`, async () => {
      ...
      // Create a VP and verify it from this credential
      await createAndVerifyPresentation(
        credentials,
        {
          resolver,
        },
    ...
    // TODO: unskip, currently doesn't work for PS
    (Name === "PS" ? test.skip : test)(
      "Holder creates a derived verifiable credential from a credential with range proofs",
      async () => {
