...
export default class PAuth {
  constructor() {
    // this.map = new BTreeMap();
    this.map = [];
  }

  add(didSig, nonce) {
    // this.map.set(didSig.did, {sig: didSig, nonce});
    // this.map.push([didSig.did, {sig: didSig, nonce}]);
    this.map.push({ sig: didSig, nonce });
  }

  toJSON() {
    // return {auths: this.map};
    return this.map;
  }
}
