...
import { FullNodeEndpoint, TestAccountURI, TestKeyringOpts, Schemes } from '../../test-constants';
...
for (const { Name, Module, Presentation, getModule } of Schemes) {
  describe(`${Name} Derived Credentials`, () => {
    ...
    test('Holder creates a derived BBS+ verifiable credential from a BBS+ credential with selective disclosure', async () => {
      ...
      const idx = await presentationInstance.addCredentialToPresent(credential, { resolver });

      // Reveal subject attributes
      await presentationInstance.addAttributeToReveal(idx, ['credentialSubject.lprNumber']);
      ...
      await presentationInstance.addAttributeToReveal(idx, ['credentialSubject.type.0']);
      await presentationInstance.addAttributeToReveal(idx, ['credentialSubject.type.1']);
      ...
      const presentation = await presentationInstance.createPresentation(presentationOptions);
