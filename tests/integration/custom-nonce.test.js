...
describe("Custom nonce", () => {
  ...
  test("Add key, controller, service endpoint, blob, BBS+ params and keys and accumulator in a batch", async () => {
    ...
    const tx1 = await dock.did.createAddKeysTx(
      [didKey],
      did1,
      did1,
      pair,
    ...
    // Add controller
    const tx2 = await dock.did.createAddControllersTx(
      [did2],
      did1,
      did1,
      pair,
    ...
    const tx3 = await dock.did.createAddServiceEndpointTx(
      spId,
      spType,
      origins,
      did1,
    ...
    const tx4 = await dock.blob.createNewTx(blob, did1, pair, {
      nonce: nonce + 4,
    });
    txs.push(tx4);
    ...
    const tx5 = await dock.bbsPlusModule.createAddParamsTx(
      addParams,
      did1,
      pair,
      { nonce: nonce + 5 },
    ...
    const tx6 = await dock.bbsPlusModule.createAddPublicKeyTx(
      pk,
      did1,
      did1,
      pair,
    ...
    const addParams1 = AccumulatorModule.prepareAddParameters(
      u8aToHex(params1.bytes),
      undefined,
      label,
    );
    const tx7 = await dock.accumulatorModule.createAddParamsTx(
      addParams1,
      did1,
      pair,
      { nonce: nonce + 7 },
    ...
    const tx8 = await dock.accumulatorModule.createAddPublicKeyTx(
      pk1,
      did1,
      pair,
      { nonce: nonce + 8 },
    ...
    const tx9 = await dock.accumulatorModule.createAddPositiveAccumulatorTx(
      id1,
      accumulated1,
      [did1, 1],
      did1,
    ...
    const tx10 = await dock.accumulatorModule.createAddUniversalAccumulatorTx(
      id2,
      accumulated2,
      [did1, 1],
      maxSize,
    ...
    expect(queriedPk.paramsRef).toEqual([DockDid.from(did1), 1]);
  ...
  test("Add 3 registries and submit revocations for all in a batch", async () => {
    ...
    const pair = new DidKeypair(dock.keyring.addFromUri(seed1), 1);
    let currentNonce = await dock.didModule.getNonceForDid(DockDid.from(did1));
    let txs = [];

    // Revoke from all 3 registries in the same transaction batch
    for (const [regId, revs, nonce] of [
      [registryId1, revokeIds1, currentNonce + 1],
    ...
    for (const [regId, revs] of [
      [registryId1, revokeIds1],
      [registryId2, revokeIds2],
      [registryId3, revokeIds3],
    ]) {
    ...
    currentNonce = await dock.didModule.getNonceForDid(DockDid.from(did1));
    txs = [];
    for (const [regId, nonce] of [
      [registryId1, currentNonce + 1],
    ...
    for (const regId of [registryId1, registryId2, registryId3]) {
      await expect(
        dock.revocation.getRevocationRegistry(regId),
  ...
  test("Add keys, service endpoints and remove several DIDs in a batch", async () => {
    ...
    let nonce = await dock.didModule.getNonceForDid(DockDid.from(did1));
    let txs = [];
    ...
    txs.push(tx1);
    const tx2 = await dock.did.createAddKeysTx(
      [new DidKey(pair4.publicKey(), vr)],
      did4,
      did1,
      pair,
      nonce + 2,
    ...
    txs.push(tx2);
    const tx3 = await dock.did.createAddKeysTx(
      [new DidKey(pair5.publicKey(), vr)],
      did5,
      did1,
      pair,
      nonce + 3,
    ...
    const tx4 = await dock.did.createAddServiceEndpointTx(
      spId3,
      spType,
      origins[0],
      did3,
    ...
    txs.push(tx4);
    const tx5 = await dock.did.createAddServiceEndpointTx(
      spId4,
      spType,
      origins[1],
      did4,
    ...
    txs.push(tx5);
    const tx6 = await dock.did.createAddServiceEndpointTx(
      spId5,
      spType,
      origins[2],
      did5,
    ...
    // Check results of all transactions
    await expect(dock.did.getDidKey(did3, 2)).resolves.toEqual(
      new DidKey(pair3.publicKey(), vr),
    );
    await expect(dock.did.getDidKey(did4, 2)).resolves.toEqual(
      new DidKey(pair4.publicKey(), vr),
    );
    await expect(dock.did.getDidKey(did5, 2)).resolves.toEqual(
      new DidKey(pair5.publicKey(), vr),
    );
    await expect(dock.did.getServiceEndpoint(did3, spId3)).resolves.toEqual({
      type: spType,
      origins: origins[0],
    });
    await expect(dock.did.getServiceEndpoint(did4, spId4)).resolves.toEqual({
      type: spType,
      origins: origins[1],
    });
    await expect(dock.did.getServiceEndpoint(did5, spId5)).resolves.toEqual({
      type: spType,
      origins: origins[2],
    });

    // Each DID adds 2 blobs
    const nonce3 = await dock.didModule.getNonceForDid(DockDid.from(did3));
    const nonce4 = await dock.didModule.getNonceForDid(DockDid.from(did4));
    const nonce5 = await dock.didModule.getNonceForDid(DockDid.from(did5));
    txs = [];

    const [
      [blobId1, blobHex1, blob1],
      [blobId2, blobHex2, blob2],
      [blobId3, blobHex3, blob3],
      [blobId4, blobHex4, blob4],
      [blobId5, blobHex5, blob5],
      [blobId6, blobHex6, blob6],
    ] = [1, 2, 3, 4, 5, 6].map((_) => {
    ...
    const tx7 = await dock.blob.createNewTx(blob1, did3, pair3, {
      nonce: nonce3 + 1,
    });
    const tx8 = await dock.blob.createNewTx(blob2, did3, pair3, {
      nonce: nonce3 + 2,
    });
    txs.push(tx7);
    txs.push(tx8);

    const tx9 = await dock.blob.createNewTx(blob3, did4, pair4, {
      nonce: nonce4 + 1,
    });
    const tx10 = await dock.blob.createNewTx(blob4, did4, pair4, {
      nonce: nonce4 + 2,
    });
    txs.push(tx9);
    txs.push(tx10);

    const tx11 = await dock.blob.createNewTx(blob5, did5, pair5, {
      nonce: nonce5 + 1,
    });
    const tx12 = await dock.blob.createNewTx(blob6, did5, pair5, {
      nonce: nonce5 + 2,
    });
    txs.push(tx11);
    txs.push(tx12);
    ...
    // Check results
    for (const [d, bi, bh] of [
      [did3, blobId1, blobHex1],
      [did3, blobId2, blobHex2],
      [did4, blobId3, blobHex3],
      [did4, blobId4, blobHex4],
      [did5, blobId5, blobHex5],
      [did5, blobId6, blobHex6],
    ]) {
      const chainBlob = await dock.blob.get(bi);
      expect(!!chainBlob).toBe(true);
      expect(u8aToHex(chainBlob[1])).toEqual(bh);
      expect(chainBlob[0]).toEqual(DockDid.from(d));
    }

    // Remove all DIDs in a batch
    nonce = await dock.didModule.getNonceForDid(DockDid.from(did1));
    txs = [];
