...
describe.skip('Fees', () => {
  ...
  const withPaidFeeMatchingSnapshot = (sendTx, applyToBefore) => withPaidFeeAssertion(
    sendTx,
  ...
  test('dids', async () => {
    ...
    await withPaidFeeMatchingSnapshot(() => dock.did.new(did, [didKey], [], false, undefined, false));
    ...
    await withPaidFeeMatchingSnapshot(() => dock.did.addServiceEndpoint(
      spId1,
      spType,
      origins1,
      did,
    ...
    await withPaidFeeMatchingSnapshot(() => dock.did.new(did1, [], [did], false));

    // Adding a new DID which doesn't control itself but controlled by 2 other controllers
    const [did2] = createDidPair(dock);
    await withPaidFeeMatchingSnapshot(() => dock.did.new(did2, [], [did, did1], false));
    ...
    await withPaidFeeMatchingSnapshot(() => dock.did.new(did3, [dk_], [did], false));
    ...
    await withPaidFeeMatchingSnapshot(() => dock.did.removeServiceEndpoint(
      spId1,
      did,
      did,
      pair,
  ...
  test('bbsPlus', async () => {
    ...
    // Remove public key
    await withPaidFeeMatchingSnapshot(() => dock.bbsPlusModule.removePublicKey(
      2,
      did,
      did,
      pair,
    ...
    // Remove params
    await withPaidFeeMatchingSnapshot(() => dock.bbsPlusModule.removeParams(
      1,
      did,
      pair,
      1,
  ...
  test('accumulator', async () => {
    ...
    await withPaidFeeMatchingSnapshot(() => dock.accumulatorModule.addParams(
      params,
      did,
      pair,
      1,
    ...
    await withPaidFeeMatchingSnapshot(() => dock.accumulatorModule.addPositiveAccumulator(
      accumulatorIdPos,
      accumulatedPos,
      [did, 1],
      did,
    ...
    await withPaidFeeMatchingSnapshot(() => dock.accumulatorModule.addUniversalAccumulator(
      accumulatorIdUni,
      accumulatedUni,
      [did, 1],
      10000,
    ...
    await withPaidFeeMatchingSnapshot(() => dock.accumulatorModule.removeAccumulator(
      accumulatorIdPos,
      did,
      pair,
      1,
  ...
  test('transfers', async () => {
    ...
    await withPaidFeeMatchingSnapshot(() => dock.signAndSend(dock.api.tx.balances.transfer(BOB, 100), false));
