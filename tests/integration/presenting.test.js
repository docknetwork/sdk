...
describe("Verifiable Presentation where both issuer and holder have a Dock DID", () => {
  ...
  beforeAll(async () => {
    ...
    cred1 = await issueCredential(
      issuerKeyDoc,
      getUnsignedCred(credId1, holder1DID),
    );

    // Issuer issues credential with id `credId2` to holder with DID `holder2DID`
    cred2 = await issueCredential(
      issuerKeyDoc,
      getUnsignedCred(credId2, holder2DID),
    );

    // Issuer issues credential with id `credId3` to holder with DID `holder3DID`
    cred3 = await issueCredential(
      issuerKeyDoc,
      getUnsignedCred(credId3, holder3DID),
    );

    // Issuer issues credential with id `credId4` to holder with DID `holder3DID`
    cred4 = await issueCredential(
      issuerKeyDoc,
      getUnsignedCred(credId4, holder3DID),
    );
  }, 90000);
