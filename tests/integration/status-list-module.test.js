...
buildTest("StatusListCredential Module", () => {
  ...
  test("Can revoke index from a status list credential", async () => {
    ...
    const [revoke, sig, nonce] =
      await dock.statusListCredentialModule.createSignedUpdateStatusListCredential(
        statusListCredId,
        cred,
        ownerDID,
        pair,
  ...
  test("Can remove a status list", async () => {
    const [remove, sig, nonce] =
      await dock.statusListCredentialModule.createSignedRemoveStatusListCredential(
        statusListCredId,
        ownerDID,
        pair,
        1,
